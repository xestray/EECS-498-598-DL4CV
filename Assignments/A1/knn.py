...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for train_idx in range(num_train):
        for test_idx in range(num_test):
            x_train_flat = x_train[train_idx].view(-1)
            x_test_flat = x_test[test_idx].view(-1)
            dists[train_idx, test_idx] = torch.square(x_train_flat-x_test_flat).sum()
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for train_idx in range(num_train):
        dists[train_idx, :] = torch.sum(torch.square(x_tr[train_idx] - x_te), dim=1).t()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_tr = x_train.reshape(num_train, -1)
    x_te = x_test.reshape(num_test, -1)
    dists = x_tr.square().sum(dim=1).reshape(-1, 1) + x_te.square().sum(dim=1).reshape(1, -1) - 2 * torch.mm(x_tr, x_te.t())
    # (a-b)**2 = a**2 + b**2 - 2*a*b
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        values, indices = torch.topk(dists[:, j], k=k, largest=False)
        values, counts = y_train[indices].unique(sorted=True, return_counts=True)
        y_pred[j] = values[counts.argmax()]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            xx_test = x_train_folds[i]
            yy_test = y_train_folds[i]
            xx_train = torch.cat([x_train_folds[j] for j in range(num_folds) if i!=j])
            yy_train = torch.cat([y_train_folds[j] for j in range(num_folds) if i!=j])

            knn = KnnClassifier(xx_train, yy_train)
            accuracy = knn.check_accuracy(xx_test, yy_test, k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    max_idx = torch.tensor(means).argmax().item()
    best_k = sorted(k_to_accuracies.items())[max_idx][0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
